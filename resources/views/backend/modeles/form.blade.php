...
<div class='row'>
    ...
    <div class="col-md-12">
        {{-- START manufacturers --}}
        <div class="form-group">
            <label>@lang('inputs.select-data', ['data' => trans('menu.manufacturers')])</label>
            <select class="select2 form-control" id="manufacturers" name="manufacturer_id" data-placeholder="--- @lang('inputs.select-data', ['data' => trans('menu.manufacturers')]) ---" required>
                <option></option>
                @foreach ($manufacturers as $id => $name)
                    <option value="{{ $id }}" @selected(isset($row) && $row->manufacturer_id == $id)>{{ $name }}</option>
                @endforeach
            </select>
            <x-validation-error input='manufacturer_id' />
        </div>
        {{-- END manufacturers --}}
    </div>
